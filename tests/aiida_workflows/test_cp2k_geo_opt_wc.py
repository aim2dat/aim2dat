"""Test for the work chain GeoOptWorkChain."""
...
@pytest.mark.skip
def test_geo_opt_fail(aiida_local_code_factory, aiida_create_structuredata):
    """Test failed geometry optimization."""
    ...
    tuple_counter = 0
    scf_level_tuples = [(0, 0, 250.0), (0, 1, 250.0), (0, 1, 250.0), (0, 1, 250.0)]
    c_opt_trust_r = [0.001, 0.001, 0.0001, 0.35]
    for process in node.called:
        if isinstance(process, cjob.CalcJobNode):
            ...
            assert (
                input_p["MOTION"]["GEO_OPT"]["BFGS"]["TRUST_RADIUS"]
...
@pytest.mark.skip
def test_geo_opt_sucess(aiida_local_code_factory, aiida_create_structuredata):
    ...
    parameters_h2o["input_parameters"]["MOTION"] = {
        "GEO_OPT": {
            "MAX_DR": 0.01,
            "RMS_DR": 0.005,
            "MAX_FORCE": 0.0025,
    ...
    # Check called processes:
    tuple_counter = 0
    scf_level_tuples = [(1, 1, 0.0), (1, 2, 0.0), (1, 2, 0.0), (1, 2, 0.0)]
    c_opt_trust_r = [0.001, 0.001, 0.0001, 0.35]
    for process in node.called:
        if isinstance(process, cjob.CalcJobNode):
            cur_tuple = scf_level_tuples[tuple_counter]
